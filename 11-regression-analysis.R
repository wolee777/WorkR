...
#       이 데이터로부터 모델을 알아내야 하는데, 주어진 데이터를 훈련 집합( training set )이라 한다.
#
...
#
#         W(기울기), b는 어떻게 찾을 수 있을까? 이 문제를 해결하려면 x, y로 구성된 데이터가 있어야 한다.
...
#
# 단순선형 회귀식은 예측값과 실제값 사이의 오차를 최소화아는 식이지만 오차가 없을 수는 없다.
...
# 
# lm()은 기본적으로 1차 방정식, 즉 직선을 사용하지만 poly라는 옵션을 사용하면 고차 방정식 적요
